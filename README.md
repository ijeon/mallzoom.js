...
**imageOverlay**|URL 스트링 1차원 배열<br>(기본값 없음)|오버레이 이미지의 URL을 배열 형식으로 입력합니다. 각 인덱스에<br>따라 \_ioid\_[인덱스]라는 클래스가 이미지에 추가됩니다.
...
**reverseZoom**|true, false<br>(기본값: false)|확대/축소 기능의 마우스 휠 이벤트 바인드 방향을 전환합니다.<br>true 값이 입력되면 마우스 휠을 내릴 때 이미지가 확대됩니다.

### 라이센스
해당 플러그인은 GPL 3.0 라이센스로 작성되었습니다. (참고: [라이센스 전문](https://opensource.org/licenses/gpl-3.0.html))

### 알려진 버그
- 레거시 브라우저에서 jQuery 3 이상 버전 사용시 addEventListener 오류가 발생할 수 있습니다.
