...
(function($){
	$.fn.mallzoom = function(options){
		...
		function imageFit(){
			var $img = $(this);
			var _img = { width: natural($img[0]).width, height: natural($img[0]).height, ratio: this.width / this.height };
			var _frame = { width: $img.parent().innerWidth(), height: $img.parent().innerHeight(), ratio: this.width / this.height };
			
			$img.css({ top:0, left:0, width:'auto', height:'auto' });
